...
# No-op if none are running. Run this manually after editing sprites/*.py when
# the in-memory Python module in a running web UI server is stale.
set -u
...
# Snapshot (pid, port) pairs before killing so restarted servers keep the same
# URLs.
INSTANCES=()
for pid in $PIDS; do
...
for entry in "${NEW_PIDS[@]}"; do
    ...
    for i in 1 2 3 4 5; do
        if curl -fs -o /dev/null "http://127.0.0.1:$port/api/status" 2>/dev/null; then
            echo "  [OK] PID $pid on port $port is healthy" >&2
            ok=$((ok+1))
            break
        fi
        sleep 1
    ...
    if ! curl -fs -o /dev/null "http://127.0.0.1:$port/api/status" 2>/dev/null; then
        echo "  [WARN] PID $pid on port $port health check timed out (see $LOG)" >&2
    fi
done
